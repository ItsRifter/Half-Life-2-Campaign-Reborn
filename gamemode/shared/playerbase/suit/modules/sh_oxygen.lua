...
if SERVER then
  ...
  -- Parameters
  local DEFAULT_SUPPLY_RATE = 1.25
  ...
  local function PlayerUnderwater(player)
      ...
      if (not HL2CR_AUX:HasPower(player) ) then
          -- Timed loop for oxygen depletion
          if player.HL2CR_AUX.breathe.tick < CurTime() then
              if (player.HL2CR_AUX.breathe.oxygen > 0) then
                  -- Deplete oxygen reserve
                  player.HL2CR_AUX.breathe.oxygen = math.Clamp(player.HL2CR_AUX.breathe.oxygen - 0.01 * 2, 0, 1)
  ...
  hook.Add("HL2CR_AuxPowerInitialize", "HL2CR_AUX_oxygen_spawn", function(player)
  ...
  hook.Add("AuxPowerTick", "HL2CR_AUX_oxygen", function(player)
