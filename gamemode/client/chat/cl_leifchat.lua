...
function chat.AddText( ... )
	...
	if Administrator.Chat ~= nil then
	else
		return
	end
...
function Administrator.Chat.ReceiveServers( )
	Administrator.Chat.ShowServers = {
		Time = RealTime( ) + 1.5,
		Height = 0,
		Alpha = 0,
		CurrentK = 0,
	...
	for i = 1, iServers do
		...
		table.insert( Administrator.Chat.ShowServers.Servers, {
			Alpha = 510,
			Name = net.ReadString( ),
			Map = net.ReadString( ),
			Info = tostring( iPlayers ) .. " " .. "" .. " [" .. iPlayersJoining .. " / " .. iMaxPlayers .. "]",
