...
SWEP.Category				= ""
SWEP.Purpose				= "Give your friends the ammo they need"
SWEP.PrintName				= "Ammo Box"
SWEP.Instructions			= "Left click to give the specific ammo"
...
SWEP.VElements = {
	["crate"] = { type = "Model", model = "models/items/item_item_crate.mdl", bone = "ValveBiped.Bip01", rel = "", pos = Vector(0, 6, 25), angle = Angle(0, 0, -90), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["crate"] = { type = "Model", model = "models/items/item_item_crate.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(9.545, 6.889, 8.72), angle = Angle(-180, -7.716, -24), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:Think()
	...
	if self.Primary.ClipSize == self.Primary.DefaultClip then return end
...
function SWEP:Initialize()
	...
	if SERVER then
		self.TimerName = "supply" .. self:EntIndex()
		local wep = self
		timer.Create(self.TimerName, 5, 0, function()
			if IsValid(wep) then
				if wep:Clip1() < wep.Primary.DefaultClip then
					wep:SetClip1(math.min(wep:Clip1() + 1, wep.Primary.DefaultClip))
				end
			end
		end)
	end	
