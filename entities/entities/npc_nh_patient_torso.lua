...
function ENT:Initialize()
	...
	self.npc = ents.Create("npc_zombie_torso")
	self.npc:SetPos(self:GetPos())
	self.npc:SetAngles(self:GetAngles())
	self.npc:Spawn()
	self.npc:Activate()
	--self.npc:SetModel("models/nh2zombies/gibs/patient_torso.mdl")
	...
	if( IsValid(self.npc)) then
		...
		self.npc:SetPos(self.npc:GetPos()+ self.npc:GetUp()*32)
...
function ENT:Think()
	...
	if self.npc:Health() <= 0 then
		...
		if game.GetMap() == "nh2c3_v2" then
			ents.FindByName("shower_doors")[1]:Fire("Unlock")
		end
	end
end