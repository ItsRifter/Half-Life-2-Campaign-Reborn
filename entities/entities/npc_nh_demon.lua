...
function ENT:Initialize()
	...
	self.npc:Spawn()
	self.npc:Activate()
	self.npc:SetModel("models/nh2zombies/creeper.mdl")
...
function ENT:Think()
	...
	if self.npc:Health() <= 0 then
		if ents.FindByClass("npc_headcrab_fast")[1] and ents.FindByClass("npc_headcrab_fast")[1]:IsValid() then
			timer.Simple(0.1, function()
				ents.FindByClass("npc_headcrab_fast")[1]:Remove()
			end)
		end
	end
end